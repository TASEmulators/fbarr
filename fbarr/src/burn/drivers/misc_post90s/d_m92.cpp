...
static void setvector_callback(int param)
{
	switch (param)
	{
		case VECTOR_INIT:	irqvector  =  0x0;	break;
...
static inline void syncronize_sound_cpu()
{
	if (m92_kludge == 3) return; 

	int rr = ((nCyclesTotal[0] / nInterleave) - VezCurrentCPU->reg.nec_ICount) + nCyclesDone[0];

	VezOpen(1);

	nCyclesDone[1] += VezRun(((rr * 4) / 5) - nCyclesDone[1]);
...
{
	float fcycles = nBurnSoundLen * ((1.00000 * nCyclesDone[1]) / nCyclesTotal[1]);
	int cycles = (int)fcycles;

//	bprintf (0, _T("%d, %d\n"), nBurnSoundLen, cycles);

	if (cycles >= nBurnSoundLen) cycles = nBurnSoundLen;
	if (ym2151_previous >= cycles) ym2151_previous = 0;
...
void __fastcall m92WriteByte(unsigned int address, unsigned char data)
{
	...
	switch (address)
	{
		...
		case 0xf9008:
			...
			m92_sprite_buffer_timer = VezCurrentCPU->reg.nec_ICount;
			VezCurrentCPU->reg.nec_ICount = 0;
			return;
...
unsigned char __fastcall m92ReadPort(unsigned int port)
{
	switch (port)
	{
		...
		case 0x08: syncronize_sound_cpu(); return sound_status[0]; 
...
static void scanline_interrupts(int scanline)
{
	if (m92_sprite_buffer_busy == 0) {
		...
		m92_sprite_buffer_timer -= 347;
		if (m92_sprite_buffer_timer > 0) {
			nCyclesDone[0] += VezRun(m92_sprite_buffer_timer);
			m92_sprite_buffer_timer = 0;
		} else {
			m92_sprite_buffer_timer = 0 - m92_sprite_buffer_timer;
			nCyclesDone[0] += VezRun(m92_sprite_buffer_timer);
		}
	}
	...
	if (scanline == m92_raster_irq_position) {
		...
		VezSetIRQLine(m92_irq_vectorbase + 8, VEZ_IRQSTATUS_ACK);
	}
...
static int DrvFrame()
{
	...
	{
		nCyclesDone[0] += VezRun((nCyclesTotal[0] / nInterleave)-m92_sprite_buffer_timer);
		if (m92_sprite_buffer_timer) m92_sprite_buffer_timer = 0;

		scanline_interrupts(i); // update at hblank?
	...
	VezOpen(1);
	nCyclesDone[1] += VezRun(nCyclesTotal[1] - nCyclesDone[1]);
	VezCurrentCPU->reg.nec_ICount = 0;
