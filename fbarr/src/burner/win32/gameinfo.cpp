...
static int GameInfoInit()
{
	...
	UINT idsString[9] = {IDS_GAMEINFO_INGAME, IDS_GAMEINFO_TITLE, IDS_GAMEINFO_FLYER, IDS_GAMEINFO_CABINET, IDS_GAMEINFO_MARQUEE, IDS_GAMEINFO_CONTROLS, IDS_GAMEINFO_PCB , IDS_GAMEINFO_ROMINFO, IDS_GAMEINFO_HISTORY };
	...
	UpdatePreview(true, hGameInfoDlg, szAppPreviewsPath);
...
static BOOL CALLBACK DialogProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	if (Msg == WM_NOTIFY) {
		...
		if (pNmHdr->code == TCN_SELCHANGE) {
			...
			if (TabPage == 0) { UpdatePreview(true, hGameInfoDlg, szAppPreviewsPath);	}
			if (TabPage == 1) { UpdatePreview(true, hGameInfoDlg, szAppTitlesPath);		}
			if (TabPage == 2) { UpdatePreview(true, hGameInfoDlg, szAppFlyersPath);		}
			if (TabPage == 3) { UpdatePreview(true, hGameInfoDlg, szAppCabinetsPath);	}
			if (TabPage == 4) { UpdatePreview(true, hGameInfoDlg, szAppMarqueesPath);	}
			if (TabPage == 5) { UpdatePreview(true, hGameInfoDlg, szAppControlsPath);	}
			if (TabPage == 6) { UpdatePreview(true, hGameInfoDlg, szAppPCBsPath);		}
