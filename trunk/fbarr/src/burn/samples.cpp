...
#define SAMPLE_DIRECTORY	"support\\samples\\"
...
void BurnSampleInit(struct sample_list *list /*sample file list*/, int nGain /*volume percentage!*/, int bAdd /*add sample to stream?*/)
{
	if (nBurnSoundRate == 0) {
		nTotalSamples = 0;
		return;
	...
	static struct sample_list *sample_list_ptr;

	{
		strcpy (setname, BurnDrvGetTextA(DRV_NAME));
		sprintf (path, "%s%s.zip", SAMPLE_DIRECTORY, setname);
	...
	sample_list_ptr = &list[0];
	while (sample_list_ptr->name != 0) {
		sample_list_ptr++;
		nTotalSamples++;
	}
	...
	for (int i = 0; i < nTotalSamples; i++) {
		sample_list_ptr = &list[i];
		sample_ptr = &samples[i];

		if (sample_list_ptr->name[0] == '0') break;

		sprintf (path, "%s%s.zip", SAMPLE_DIRECTORY, setname);

		destination = NULL;
		length = 0;
		ZipLoadOneFile((const char*)path, (const char*)sample_list_ptr->name, &destination, &length);

		if (length) {
			make_raw((unsigned char*)destination, length);

			sample_ptr->flags = sample_list_ptr->flags;
		} else {
