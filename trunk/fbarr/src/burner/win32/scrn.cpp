...
static void UpdatePreviousGameList()
{
	...
	if (szTmp) free(szTmp); // free memory
			...
			if (bDrvOkay && !kNetGame) {
				if (StatedLoad(nSavestateSlot) == 0) {
					if(nReplayStatus) {
						...
						VidSNewShortMsg(L"state panda"); //TODO: does this actually do anything?
					}
				} else {
...
//			VidRedraw();
		...
		case MENU_AUTOSUPPORTDOWNLOAD:
			bEnableAutoSupportFileDownload = !bEnableAutoSupportFileDownload;
			break;
