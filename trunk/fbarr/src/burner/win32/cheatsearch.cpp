...
static inline void searchForAddress(CheatComparisonType cmp, CheatDataSize size, UINT32 value, UINT8 update)
{
	...
	for (unsigned int i = 0; i < cheatSearchInfo.size - l; i++)
	{
		if (TEST_BIT(cheatSearchInfo.ALL_BITS, i) && _C(cmp, (INT32)i, (INT32)value)) {
			if (update) {
				cheatSearchInfo.CRAM[i] = CheatSearchGet(i);
			}
		} else {
			...
			case IDC_C_WATCH:
				{
					...
					TCHAR buf[22];
					LVITEM lvi;
					ITEM_QUERY(lvi, IDC_ADDYS, 0, buf, 20);
					if(lvi.iItem != -1)
					{
						scanAddress(buf,&address);
						memset(buf, 0, 20);
						swprintf(buf, _T("%X"), address);
						AddMemWatch(_TtoA(buf));
					}
				}
				break;
				...
				if (BST_CHECKED == IsDlgButtonChecked(hwndDlg, IDC_ENTERED)
					...
					if (use_entered == 2) {
						...
						if (ret != 1 || !testRange(val_type, bytes, value)) {
							MessageBox(hwndDlg, _T("Please enter a valid value for a search!"), _T("FBA Cheat Error"), MB_OK);
							return TRUE;
						}
...
#endif
{
	...
	cheatSearchDlg = FBACreateDialog(hAppInst, MAKEINTRESOURCE(IDD_CHEAT_SEARCH), NULL, (DLGPROC)CheatSearchDialogProc);
