...
static struct { wchar_t pMsgText[32]; COLORREF nColour; int nPriority; unsigned int nTimer; } VidSTinyMsg = {L"", 0, 0, 0};
...
static int VidSInitTinyMsg(int nFlags)
{
	...
	TinyMsgFont = CreateFont(14, 0, 0, 0, FW_BOLD, 0, 0, 0, 0, 0, 0, ANTIALIASED_QUALITY, FF_SWISS, L"Lucida");
	VidSTinyMsg.nTimer = 0;
...
static int VidSInitChat(int /*nFlags*/)
{
	...
	nChatFontSize = 20; // used for cheat search only at the minute
...
static void VidSDisplayTinyMsg(IDirectDrawSurface7* pSurf, RECT* pRect)
{
	if (VidSTinyMsg.nTimer) {
		RECT src = { 0, 0, 96, 20 };
		RECT dest = { pRect->left + 4, pRect->top + 4, pRect->left + 100, pRect->top + 24 };
...
int VidSNewTinyMsg(const TCHAR* pText, int nRGB, int nDuration, int nPriority)	// int nRGB = 0, int nDuration = 0, int nPriority = 5
{
	...
	if (nRGB) {
		...
		VidSTinyMsg.nColour = RGB(0xFF, 0xFF, 0xFF);
	}
	if (nDuration) {
...

	{
		...
		// Print a black shadow
		SetTextColor(hDC, 0);
		TextOut(hDC, 1, 1, VidSTinyMsg.pMsgText, wcslen(VidSTinyMsg.pMsgText));
		// Print the text on top
		SetTextColor(hDC, VidSTinyMsg.nColour);
		TextOut(hDC, 1 - 1, 1 - 1, VidSTinyMsg.pMsgText, wcslen(VidSTinyMsg.pMsgText));
