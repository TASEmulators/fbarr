...
int VidSoftFXInit(int nBlitter, int nRotate)
{
	...
	if (nSoftFXBlitter >= 5 && nSoftFXBlitter <= 7) {		// Initialize the 2xSaI engine
	...
	if (nSoftFXBlitter >= 8 && nSoftFXBlitter <= 10) {
...
void VidSoftFXApplyEffect(unsigned char* ps, unsigned char* pd, int nPitch)
{
	...
	switch (nSoftFXBlitter) {
		...
		case 3: {
			if (nVidImageDepth == 15) {
				_2xpm_15(ps, pd, (unsigned long)nSoftFXImagePitch, (unsigned long)nPitch, (unsigned long)nSoftFXImageWidth, (unsigned long)nSoftFXImageHeight, nVidImageDepth);
			}
			else if (nVidImageDepth == 16) {
				_2xpm_16(ps, pd, (unsigned long)nSoftFXImagePitch, (unsigned long)nPitch, (unsigned long)nSoftFXImageWidth, (unsigned long)nSoftFXImageHeight, nVidImageDepth);
			}
			break;			
		}
		case 4: {
		...
		case 5: {											// Super Eagle blitter (15/16BPP only)
		...
		case 6: {											// 2xSaI blitter (15/16BPP only)
			unsigned char* px = pSoftFXXBuffer + nSoftFXImagePitch;
		...
		case 7: {											// Super 2xSaI blitter (15/16BPP only)
			unsigned char* px = pSoftFXXBuffer + nSoftFXImagePitch;
